...
def gSearch(query, chat_id):
	...
	for h in hits: 
		bot.sendMessage(chat_id, str(h['url']))
		...
		if '/hello' in command:
			...
			if (len(command) > 10):
				try:
					...
					bot.sendMessage(chat_id, "Can you be a little more especific?")
			...
			try:
				...
				observation = owm.weather_at_place(command[9:] + ', Brasil')
				weatherResult = json.loads(observation.to_JSON())
				temperature = str(round(float(weatherResult['Weather']['temperature']['temp']) - 273.15, 2))
				city = weatherResult['Location']['name']
				country = weatherResult['Location']['country']

				bot.sendMessage(chat_id, "Está fazendo " + temperature + "°C agora em " + city + ", " + country + ".\n\n")
			except IndexError:
				print('Default Weather search')
				observation = owm.weather_at_place('Sobral, Ceara')
				weatherResult = json.loads(observation.to_JSON())
				temperature = str(round(float(weatherResult['Weather']['temperature']['temp']) - 273.15, 2))
				city = weatherResult['Location']['name']
				country = weatherResult['Location']['country']
...
if __name__ == '__main__':
	...
	WEATHER_API = sys.argv[2]  # get APIkey from command-line
